...
const HomeElement = () => {
    const [visibleTable, setVisibleTable] = useState('carousel'); // Show carousel table by default
    ...
    const [carouselImages, setCarouselImages] = useState([]); // Store carousel images
    const [selectedImages, setSelectedImages] = useState([]); 
    const [imagePreviews, setImagePreviews] = useState([]); // Store image preview URLs
    const [link, setLink] = useState(''); // Store link input value
    const [editingCarouselId, setEditingCarouselId] = useState(null); // Track the currently edited carousel item

    const openCarouselModal = (carouselId = null) => {
        setEditingCarouselId(carouselId); // Set the ID of the carousel being edited
        if (carouselId) {
            ...
            if (carousel) {
                ...
                setLink(carousel.link); // Set the link input value
            }
        }
            ...
            <div className="page-content py-6">
                <Container fluid>
                            ...
                            <table className="min-w-full border-collapse border border-gray-300">
                                ...
                                <tbody>
                                    {carouselImages.map((carousel, index) => (
                                        <tr key={carousel._id}>
                                            <td className="border border-gray-300 p-3">{index + 1}</td>
                                            <td className="border border-gray-300 p-3">Dipro</td>
                                            <td className="border border-gray-300 p-3">
                                                {carousel.images.map((image, i) => (
                                                    <img key={i} src={`http://localhost:5000/${image}`} alt="carousel" className={`w-20 h-20 object-cover ${carousel.active ? '' : 'opacity-50'}`} />
                                                ))}
                    ...
                    {isVideoModalOpen && (
                        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                            <div className="bg-white p-6 rounded-md shadow-md w-full max-w-md">
                                <h2 className="text-xl font-bold mb-4">Add Multiple Videos</h2>
                                <form>
                                    <input type="file" multiple accept="video/*" className="mb-4 w-full" />
                                    <button
                                        type="button"
