...
const AddOthersAsset = () => {
    ...
    const fetchAssets = async () => {
        try {
            const response = await fetch(`${baseUrl}/api/others-asset`);
...
const handleSubmit = async (e) => {
    ...
    try {
        if (editMode && selectedAssetId) {
            // Edit existing asset
            const response = await fetch(`${baseUrl}/api/others-asset/${selectedAssetId}`, {
                method: 'PUT',
            ...
            const response = await fetch(`${baseUrl}/api/others-asset/create`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(newAsset),
            });
                ...
                <tbody>
                    {assets.map((asset) => (
                        <tr key={asset._id}>
                            ...
                            <td className="py-2 px-4 border">{asset.price}</td>
                            <td className="py-2 px-4 border">{asset.price * asset.quantity}</td>
