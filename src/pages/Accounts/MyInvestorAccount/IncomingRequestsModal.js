...
const IncomingRequestModal = ({ show, handleClose,myRequests,allFetchingFun }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-50">
            <div className="bg-white w-[700px] p-5 rounded-lg shadow-lg">
                ...
                {
                    myRequests.map((mq,index) =>
                        <div className="my-2">
                            <div className="shadow-md flex justify-between items-center rounded-md bg-blue-50">
                              <p className="text-lg p-2">{index+1}) <span className="text-green-500">{mq.senderId.fullName}</span> Request to {mq.type} <span className="text-red-500">{mq.amount} ৳</span> </p>  
